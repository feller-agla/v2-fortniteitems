...
export default function PartnerLayout({ children }) {
  ...
  useEffect(() => {
    ...
    const allowed = user && (profile?.role === 'partner' || profile?.role === 'admin');
    if (!allowed) {
    ...
    return (
      <div className="min-h-screen bg-[#091C3E] flex items-center justify-center">
        <div className="text-center">
          <div className="text-3xl font-display text-fortnite-yellow animate-pulse mb-4 tracking-widest">AUTHENTIFICATION...</div>
          <p className="text-gray-400 font-bold text-sm uppercase tracking-widest">Vérification de votre accès partenaire</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#091C3E] flex items-center justify-center p-4">
        <div className="bg-[#051024] border-2 border-rarity-marvel rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
          <div className="text-rarity-marvel text-6xl mb-4">🔒</div>
          <h1 className="text-2xl font-display text-white mb-2 tracking-widest text-3d">ACCÈS PARTENAIRE</h1>
          <p className="text-gray-400 font-bold text-sm uppercase mb-6">
            {user ? "Votre compte n'a pas le statut partenaire." : "Connectez-vous avec votre compte partenaire."}
          </p>
    ...
    <div className="min-h-screen bg-[#091C3E] text-white flex flex-col md:flex-row font-sans">
      ...
      <aside className="w-full md:w-64 bg-[#051024] border-r-4 border-b-4 md:border-b-0 border-[#1A3E7A] flex flex-col shrink-0 z-20">
        <div className="p-6 border-b-2 border-white/5 text-center md:text-left">
          <Link href="/partner" className="group flex flex-col items-center gap-2 transition-transform hover:scale-105 active:scale-95">
            <img 
        ...
        <nav className="flex-1 p-4 space-y-2 flex md:flex-col overflow-x-auto md:overflow-visible overflow-y-hidden md:overflow-y-auto">
          <Link href="/partner" className="flex items-center gap-3 p-3 text-white hover:bg-white/10 rounded-xl transition-colors whitespace-nowrap group">
            <ChartBarIcon className="w-6 h-6 text-gray-400 group-hover:text-fortnite-yellow" />
            <span className="font-bold text-sm tracking-widest">MES STATS</span>
          </Link>
        </nav>
        ...
        <div className="p-4 border-t-2 border-white/5">
          <Link href="/" className="flex items-center gap-3 p-3 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl transition-colors group">
            <ArrowLeftEndOnRectangleIcon className="w-6 h-6 group-hover:text-fortnite-yellow" />
